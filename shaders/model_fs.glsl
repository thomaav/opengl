...
void main()
{
	vec3 ambient = light0.ambient * vec3(texture(texture_diffuse1, texture_coords));

	vec3 norm_normal = normalize(normal);
	vec3 norm_light_direction = normalize(light0.position - fragment_position);
	float diff = max(dot(norm_normal, norm_light_direction), 0.0f);
	vec3 diffuse = light0.diffuse * diff * vec3(texture(texture_specular1, texture_coords));
	...
	vec3 specular = light0.specular * spec * vec3(texture(texture_specular1, texture_coords));
